...
class CFileHelper
{
        ...
        while (($file = readdir($folder)) !== false)
        {
            ...
            if (self::validatePath($base, $file, $isFile, $fileTypes, $exclude))
            {
                if ($isFile)
                {
                    ...
                    self::copyDirectoryRecursive($path, $dst . DIRECTORY_SEPARATOR . $file,
                        $base . '/' . $file, $fileTypes, $exclude, $level - 1, $options);
                }
            }
        }
        closedir($folder);
        ...
        while (($file = readdir($handle)) !== false)
        {
            ...
            if (self::validatePath($base, $file, $isFile, $fileTypes, $exclude))
            {
                if ($isFile) {
                    ...
                    $list = array_merge($list, self::findFilesRecursive($path,
                        $base . '/' . $file, $fileTypes, $exclude, $level - 1));
                }
            }
        }
        closedir($handle);
    ...
    public static function getMimeTypeByExtension($file, $magicFile = null)
    {
        static $extensions;
        if ($extensions === null) {
            $extensions = $magicFile === null ? require(
                Yii::getPathOfAlias('system.utils.mimeTypes') . '.php') : $magicFile;
        }
